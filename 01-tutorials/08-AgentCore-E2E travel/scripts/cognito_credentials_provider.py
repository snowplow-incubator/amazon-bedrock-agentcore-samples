...
def store_provider_name_in_ssm(provider_name: str):
    ...
    param_name = "/app/travelagent/agentcore/cognito_provider"
    try:
        ssm.put_parameter(
...
def create_cognito_provider(provider_name: str) -> dict:
    ...
    try:
        ...
        client_id = get_ssm_parameter(
            "/app/travelagent/agentcore/machine_client_id"
        )
        click.echo(f"✅ Retrieved client ID: {client_id}")

        client_secret = get_ssm_parameter(
            "/app/travelagent/agentcore/cognito_secret"
        )
        click.echo(f"✅ Retrieved client secret: {client_secret[:4]}***")

        issuer = get_ssm_parameter(
            "/app/travelagent/agentcore/cognito_discovery_url"
        )
        auth_url = get_ssm_parameter("/app/travelagent/agentcore/cognito_auth_url")
        token_url = get_ssm_parameter(
            "/app/travelagent/agentcore/cognito_token_url"
        )
